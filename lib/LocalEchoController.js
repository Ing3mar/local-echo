        ...
        case "\t": // TAB
          if (this._autocompleteHandlers.length > 0) {
            ...
            if (candidates.length === 0) {
              ...
              // Only take the last fragment, not the whole input
              const lastFragment = getLastToken(inputFragment);

              // search for a shared fragement
              const sameFragment = getSharedFragment(lastFragment, candidates);

              // if there's a shared fragement between the candidates
              // print complete the shared fragment
              if (sameFragment) {
                this.handleCursorInsert(
                  sameFragment.substr(lastFragment.length)
                );
              }
